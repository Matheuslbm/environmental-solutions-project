import { Link } from 'react-router-dom';
import { ABOUT_CONTENT } from '../data/content';

export function About() {
  return (
    <div className="flex flex-col">
      
      {/* 1. HERO SECTION (Topo padrão das páginas internas) */}
      <section className="relative bg-blue-900 py-24 text-center text-white">
        {/* Overlay para simular o efeito de água azulada */}
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500 to-blue-800 opacity-90"></div>
        
        <div className="relative container mx-auto px-4 z-10">
          <h1 className="mb-2 text-4xl font-bold uppercase tracking-wider sm:text-5xl font-serif">
            Sobre a AWFD
          </h1>
          <p className="text-lg text-blue-100 font-light tracking-wide">
            Quem somos
          </p>
        </div>
      </section>

      {/* 2. CONTEÚDO PRINCIPAL (Imagem + Texto) */}
      <section className="bg-white py-20">
        <div className="container mx-auto px-4">
          <div className="flex flex-col items-center gap-12 md:flex-row md:items-start">
            
            {/* Lado Esquerdo: A FOTO */}
            <div className="w-full md:w-1/2">
              <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg shadow-xl">
                {/* Placeholder cinza enquanto não tem a foto real */}
                <div className="flex h-full w-full items-center justify-center bg-gray-200 text-gray-500">
                  [Foto: João Mariz no Painel]
                </div>
                {/* Quando colocar a imagem real:
                <img 
                  src="/images/joao-mariz.jpg" 
                  alt="João Mariz fundador da AWFD" 
                  className="h-full w-full object-cover"
                />
                */}
              </div>
            </div>

            {/* Lado Direito: O TEXTO */}
            <div className="w-full md:w-1/2 text-center md:text-left">
              <h2 className="mb-8 text-2xl font-bold uppercase text-black sm:text-3xl font-serif">
                {ABOUT_CONTENT.title}
              </h2>
              
              <p className="text-lg leading-relaxed text-gray-600 text-justify">
                {ABOUT_CONTENT.text}
              </p>
            </div>

          </div>
        </div>
      </section>

      {/* 3. CALL TO ACTION (Faixa Azul) */}
      <section className="bg-blue-100 py-16 text-center">
        <div className="container mx-auto px-4">
          <h2 className="mb-8 text-2xl font-bold uppercase text-blue-900 font-serif">
            Contate um de nossos profissionais
          </h2>
          <Link
            to="/contato"
            className="inline-block rounded-full bg-blue-500 px-10 py-4 text-sm font-bold text-white shadow-lg transition transform hover:bg-blue-600 hover:-translate-y-1"
          >
            CLIQUE AQUI
          </Link>
        </div>
      </section>

    </div>
  );
}